...
export default async function handler(req, res) {
  ...
  // Portador NUVEMSHOP DECORADA — captura todas as recebidas de Porcelana Decorada
  const PORTADOR_ID = '14888102402';
  const startPage = parseInt(req.query.startPage || '26', 10);
  const endPage   = parseInt(req.query.endPage   || '45', 10);
  ...
  for (let pagina = startPage; pagina <= endPage; pagina++) {
    ...
    for (const item of items) {
      if ((item.vencimento || '').startsWith(prefixo) && !idsVistos.has(item.id)) {
